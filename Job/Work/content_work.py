...
class ContentJob(object):
    ...
    def process(self, task=None):
        ...
        r_mongo = mongo_conn.contents.find(
            {'relationship': {'$elemMatch': {'mediaId': task.get("mediaId"), "platform": task.get("platform")}}})
        if r_mongo.count() > 0:
        ...
        if task.get("actor") and task.get("director"):
            ...
            print("task:--contentName:%s---contentType:%s---director:%s---actor:%s---area:%s-" % (
            task.get("contentName"), task.get("contentType"), task.get("director"), task.get("actor"),
            task.get("area")))
            regx = {}
            if task.get("director") and task.get("director") != "":
                regx["directors"] = re.compile(u"(" + "|".join(process_actor(task.get("director")).split(',')) + ")",
                                               re.IGNORECASE)  # 匹配至少有一个directors相交的
            if task.get("actor") and task.get("actor") != "":
                regx['actors'] = re.compile(u"(" + "|".join(process_actor(task.get("actor")).split(',')) + ")",
                                            re.IGNORECASE)  # 匹配至少有一个starring相交的
            if task.get("year"):
                regx['year'] = re.compile(u"(" + "|".join(process_actor(task.get("year")).split(',')) + ")",
                                          re.IGNORECASE)  # 匹配至少有一个year
            if task.get("area"):
                regx['area'] = re.compile(u"(" + "|".join(area_process(task.get("area"))) + ")",
                                          re.IGNORECASE)  # 匹配至少有一个year
            ...
            if contents.count() == 0:
                ...
                for mon in contents:
                    ...
                    print("-sim:%s--title:%s--contentName:%s---contentType:%s---director:%s---actor:%s---area:%s-" % (
                    sim, mon['title'], task.get("contentName"), task.get("contentType"), task.get("director"),
        ...
        if c[0].get("relationship"):
            ...
            if f == True:
                ...
                result = mongo_conn.contents.update_one({"_id": _id}, {"$push": {"relationship": {"platform": task.get(
                    "platform"), "mediaId": task.get("mediaId"), "CPID": task.get("CPID"),
                    "mediaName": task.get("contentName")}}})
                print(result)
        else:
            result = mongo_conn.contents.update_one({"_id": _id}, {"$push": {"relationship": {"platform": task.get(
                "platform"), "mediaId": task.get("mediaId"), "CPID": task.get("CPID"),
