...
def requests_get(url, session = None,headers=None, data={}, timeout=10,cookies=None):
    retry = 5
    print(url)
    if not headers:
        headers = {}
...
def requests_post(url, session = None,headers=None, data=None, timeout=10,cookies=None):
    retry = 5
    if not headers:
        headers = {}
        ua = user_agent.generate_navigator(os=None, navigator=None, platform=None, device_type=None)
